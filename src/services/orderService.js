...
// Filter order by status and Pagination
...
let filterOrderPagination = (req) => {
    return new Promise(async (resolve, reject) => {
        try {
            let { page, size, status } = req;
            const { limit, offset } = getPagination(page, size);

            let orders = '';
            let response;
            if (status === 'S0') {
                response = await db.Order.findAndCountAll({
                    limit,
                    offset,
                    attributes: {
                        exclude: ['createdAt', 'updatedAt'],
            ...
            orders = getPagingData(response, page, limit);
...
let searchOrder = (req) => {
    return new Promise(async (resolve, reject) => {
        try {
            let { page, size, code } = req;
            const { limit, offset } = getPagination(page, size);

            let orders = '';
            const condition = code ? { code: { [Op.like]: `%${code}%` } } : null;
            let response = await db.Order.findAndCountAll({
                where: condition,
                limit,
                offset,
                attributes: {
...
const beginingOfDay = (options = {}) => {
    ...
    return new Date(1000 * Math.floor((date - hour * 3600000 - minute * 60000 - second * 1000) / 1000));
};

const endOfDay = (...args) => new Date(beginingOfDay(...args).getTime() + 86399999);
