                    ...
                    include: [
                        {
                            ...
                            attributes: [[db.sequelize.fn('SUM', db.sequelize.col('qty')), 'count']],
                        },
                    ],
                });
            }
...
let GetPaginationProducts = (req) => {
    return new Promise(async (resolve, reject) => {
        try {
            let { page, size, title } = req;
            const condition = title ? { name: { [Op.like]: `%${title}%` } } : null;
            ...
            products = getPagingData(response, page, limit);
